...
static bool FindPythonInstallation(char* outPath, size_t maxLen)
{
    ...
    if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_PROGRAM_FILES, NULL, 0, basePath))) {
        if (ScanBasePathForPython(basePath, outPath, maxLen)) return true;
    }

    // 3. Check Program Files (x86)
    if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_PROGRAM_FILESX86, NULL, 0, basePath))) {
        if (ScanBasePathForPython(basePath, outPath, maxLen)) return true;
    }

    // 4. Check Local AppData
    if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, basePath))) {
...
static void AutoSetPythonHome()
{
    ...
    if (GetModuleFileNameA(NULL, currentExe, MAX_PATH)) {
        ...
        if (filename) filename++; else filename = currentExe;
        
        if (strcasecmp(filename, "python.exe") == 0 || strcasecmp(filename, "pythonw.exe") == 0) {
    ...
    if (!found) {
        ...
        if (FindPythonExePath(pythonExe, MAX_PATH)) {
            ...
            pythonHome[MAX_PATH - 1] = '\0';
            PathRemoveFileSpecA(pythonHome);
            if (IsValidPythonHome(pythonHome, pythonHome, MAX_PATH)) found = true;
        }
    }
    ...
    if (found) {
        ...
        snprintf(pythonPath, sizeof(pythonPath), "%s\\Lib;%s\\Lib\\site-packages", pythonHome, pythonHome);
...
static bool SetupStdoutStderrToLog(char *outLogPath)
{
    ...
    if (outLogPath) strncpy(outLogPath, logPath, MAX_PATH - 1);
...
DWORD WINAPI hookImpl(LPVOID lpParam) {
    ...
    if (hPyDll) {
        ...
        // 2. XP/Loop style check for python313.dll down to python36.dll
        // This keeps the "3%" format logic the user wants
        for (int i = 13; i >= 6; i--) {
    ...
    auto PyImport_ImportModule = (PyImportModuleFunc)GetProcAddress(hPyDll, "PyImport_ImportModule");
    auto Py_DecRef = (Py_DecRefFunc)GetProcAddress(hPyDll, "Py_DecRef");
    auto PyGILState_Ensure = (PyGILState_EnsureFunc)GetProcAddress(hPyDll, "PyGILState_Ensure");
    auto PyGILState_Release = (PyGILState_ReleaseFunc)GetProcAddress(hPyDll, "PyGILState_Release");
    auto PyErr_Print = (PyErr_PrintFunc)GetProcAddress(hPyDll, "PyErr_Print");
    auto PyRun_SimpleString = (PyRun_SimpleStringFunc)GetProcAddress(hPyDll, "PyRun_SimpleString");

    if (!PyImport_ImportModule || !PyGILState_Ensure || !PyGILState_Release) {
        MessageBoxA(NULL, "Cannot load Python C-API functions", "Hook Error", MB_ICONEXCLAMATION);
        return 1;
    }
    ...
    if (PyRun_SimpleString) {
        ...
        snprintf(pycmd, sizeof(pycmd),
            ...
            "            print('Added to sys.path (site-packages):', site_packages)\n"
            "    \n"
    ...
    if (hook_module) {
        ...
        if (g_logFile) {
            fprintf(g_logFile, "[HOOK] Failed to import " PYMODULE_NAME " - check traceback above\n");
            fflush(g_logFile);
        }
